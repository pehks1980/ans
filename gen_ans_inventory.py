...
host_mappings = {
    "stage": {"hostname": "linuxS", "ip_key": 0},  # Use the first IP for "stage"
    "prod": {"hostname": "linuxP", "ip_key": 1}   # Use the second IP for "prod"
}
    ...
    try:
        with open(output_file, "w") as f:
            for group, data in host_mappings.items():
                ...
                f.write(f"[{group}]\n")
                f.write(f"{hostname}  ansible_host={ip} ansible_user=ubuntu ansible_ssh_private_key_file={ssh_key_path}\n\n")
